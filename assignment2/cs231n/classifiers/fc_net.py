...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(hidden_num):
            if self.normalization == 'batchnorm':
                scores, cache = affine_batchnorm_relu_forward(scores,
                                                              self.params['W%d'%(i+1)],
                                                              self.params['b%d'%(i+1)],
            ...
            if self.use_dropout:
                ...
                cache_history.append(dropout_cache)
        ...
        while i >= 0:
            ...
            if self.normalization == "batchnorm":
                dout, grads['W%d'%(i+1)], grads['b%d'%(i+1)], grads['gamma%d'%(i+1)], grads['beta%d'%(i+1)] = affine_batchnorm_relu_backward(dout, cache_history.pop())
            else:
...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    performs an affine transform follow by batchnorm and then relu
    """
    a, fc_cache = affine_forward(x, w, b)
    normalised, bn_cache = batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(normalised)
