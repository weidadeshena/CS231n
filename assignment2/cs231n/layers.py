...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # cache
        cache = (x, mu, var, std, x_hat, gamma, beta, eps, N, D, layernorm)
...
def batchnorm_backward(dout, cache):
    ...
    # get data
    x, mu, var, std, x_hat, gamma, beta, eps, N, D, layernorm = cache
    
    # grad for beta & gamma
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_hat, axis=0)
    
    # grad for x, computed from computational graph...
    dx_hat = dout * gamma
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * x_hat, axis=layernorm)
    
    # grad for x
    df_dx_hat = dout * gamma
    sum_df_dx_hat = np.sum(df_dx_hat, axis=0)
    dx = df_dx_hat * N - np.sum(df_dx_hat, axis=0) -np.sum(df_dx_hat*x_hat, axis=0) * x_hat
    dx /= N*std
